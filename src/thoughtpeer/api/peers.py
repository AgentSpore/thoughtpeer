...
async def find_similar(query: PeerSearchQuery, db: DbDep):
    return await matching_service.find_similar(db, query)


@router.post("/solutions", response_model=list[PeerSolution])
async def find_solutions(query: PeerSearchQuery, db: DbDep):
    return await matching_service.find_solutions(db, query)


@router.post("/share", status_code=201)
async def share_to_pool(data: PeerShareRequest, db: DbDep):
    return await peer_service.share_to_pool(db, data)


@router.get("/stats")
async def pool_stats(db: DbDep):
